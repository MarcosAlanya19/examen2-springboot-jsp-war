  ...
  <body>
    <div class="container mt-5">
      <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <h1 class="h2 mb-0">Mesas disponibles</h1>
        <img
      ...
      <c:if test="${empty tables}">
        <p class="lead">No hay mesas disponibles.</p>
      </c:if>
        ...
        <table class="table table-hover text-center">
          ...
          <tbody>
            <c:forEach items="${tables}" var="t">
              <tr>
                <td><a href="/${AppConfig.ROUTE_INDEX_TABLE}/${t.id}/detail"><c:out value="${t.name.toUpperCase()}" /> </a></td>
                <td><c:out value="${t.number}" /></td>
                <td><fmt:formatDate value="${t.createdAt}" pattern="MMMM d HH:mm" /></td>
                <td><form
                  action="${pageContext.request.contextPath}/${AppConfig.ROUTE_INDEX_TABLE}/${t.id}/assign"
                  method="post"
                >
                  <button type="submit" class="btn btn-primary">Agarrar mesa</button>
                </form></td>
              </tr>
            </c:forEach>
          </tbody>
      ...
      <p class="mt-3"><a class="btn btn-primary" href="/${AppConfig.ROUTE_HOME}">Volver al inicio</a></p>
    </div>
